...
class _regex:
    def __init__(self):
        # Hash Regex
        self.sha256_regex = re.compile(r"[A-Fa-f0-9]{64}")
        self.sha1_regex = re.compile(r"[A-Fa-f0-9]{40}")
        self.md5_regex = re.compile(
            r"\b(?!^[\d]*$)(?!^[a-fA-F]*$)([a-f\d]{32}|[A-F\d]{32})")
